...
for ((i=0; i<num_data_collectors; i++)); do
  CUDA_VISIBLE_DEVICES=0 nohup python explore_council_long_horizon.py -N $N -T 10 -i $i -o ./data/$1 -d cuda:0 &
  pids+=($!)  # Store the PID of the background process
done

for ((i=0; i<num_data_collectors; i++)); do
  CUDA_VISIBLE_DEVICES=1 nohup python explore_council_long_horizon.py -N $N -T 10 -i $((i + num_data_collectors)) -o ./data/$1 -d cuda:0&
  pids+=($!)  # Store the PID of the background process
done
